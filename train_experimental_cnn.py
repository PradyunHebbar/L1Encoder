...
class HGCL1(tf.keras.Model):
    ...
    def set_trainable(self, d, _value=None):
        if type(d) != dict:
...
figpaths = []
def generate_and_save_images(model, test_sample, level="ROC"):
    '''
    ...
    zero_entries = test_sample[np.where(test_sample.max(axis=(1, 2, 3, 4, 5)) == 0)]
    ...
    z = model.encode(test_sample, level=level, data_format="STAGE2")
    predictions = np.array(model.decode(z, level=level, data_format="STAGE2"))

    #manually reshape to triplet shape
    test_sample = test_sample.reshape((-1, 3, 3, 4, 4, 1))
    predictions = predictions.reshape((-1, 3, 3, 4, 4, 1))

    # reshape for visualisation    
    test_sample = np.transpose(test_sample, [0, 1, 3, 2, 4, 5])
    predictions = np.transpose(predictions, [0, 1, 3, 2, 4, 5])
    test_sample = test_sample.reshape(-1, 12, 12, 1)
    predictions = np.reshape(predictions, (-1, 12, 12, 1))

    # manually reshape and select only test samples with non-zero entries
    positive_indexes = np.where(test_sample.max(axis=(1, 2, 3)) > 0)
    zero_indexes = np.where(test_sample.max(axis=(1, 2, 3)) == 0)

    #shuffle and select subset
    test_sample_positives = test_sample[positive_indexes]
    predictions_positives = predictions[positive_indexes]
    perm_positives = [x for x in range(test_sample_positives.shape[0])]
    np.random.seed(0)
    np.random.shuffle(perm_positives)    
    test_sample_positives = test_sample_positives[perm_positives]
    predictions_positives = predictions_positives[perm_positives]
    test_sample_positives = test_sample_positives[0:num_examples_to_generate]
    predictions_positives = predictions_positives[0:num_examples_to_generate]


    test_sample_zeros = test_sample[zero_indexes]
    predictions_zeros = predictions[zero_indexes]
    perm_zeros = [x for x in range(test_sample_zeros.shape[0])]
    np.random.seed(0)
    np.random.shuffle(perm_zeros)    
    test_sample_zeros = test_sample_zeros[perm_zeros]
    predictions_zeros = predictions_zeros[perm_zeros]
    test_sample_zeros = test_sample_zeros[0:4]
    predictions_zeros = predictions_zeros[0:4]

    # merging everything
    test_sample = np.append(test_sample_positives, test_sample_zeros, axis=0)
    predictions = np.append(predictions_positives, predictions_zeros, axis=0)
...
try:
    ...
    print("No model file exists. Model is not loaded but initialised.")
    model.initialize_networks()

if model.current_epoch==0:
    generate_and_save_images(model, test_sample, level="STAGE2")
else:    
    l2_loss_test = evaluate_loss(model, test_dataset, model.TRAIN_STAGE)
    print
    print("Test loss before training is:", l2_loss_test)
    print
model.TRAIN_STAGE = "UNDEFINED"
for epoch in range(model.current_epoch + 1, args.NEpochs + 1):
    NEW_TRAIN_STAGE = model.TRAIN_STAGE
    if epoch >= 1:
        NEW_TRAIN_STAGE = "STAGE2"
        trainable_dict = {
            "ENC":
                {"ROC": True,
                 "MODULE": True,
    ...
    if model.TRAIN_STAGE != NEW_TRAIN_STAGE:
        model.TRAIN_STAGE = NEW_TRAIN_STAGE
        model.set_trainable(trainable_dict)
    
    start_time = time.time()
    for train_x in train_dataset:
        train_step(model, train_x, optimizer, model.TRAIN_STAGE)
        model.min_step += 1
        l2_loss_train = compute_loss(model, train_x, model.TRAIN_STAGE)
    ...
    print('Phase: {}, Epoch: {}, Train set L2: {}, Test set L2: {}, time elapse for current epoch: {}s'
          .format(model.TRAIN_STAGE, epoch, l2_loss_train, l2_loss_test, round(end_time - start_time, 1)))
    generate_and_save_images(model, test_sample, level=model.TRAIN_STAGE)
